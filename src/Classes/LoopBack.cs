...
namespace XWall.LoopBack
{
    ...
    public class LoopUtil
    {
        ...
        public bool OnekeyDisableIsolation(bool disable)
        {
             LoopUtil _loop;
             List<LoopUtil.AppContainer> appFiltered = new List<LoopUtil.AppContainer>();
             _loop = new LoopUtil();
             foreach (LoopUtil.AppContainer app in _loop.Apps)
             {
                 app.LoopUtil = disable;
                 appFiltered.Add(app);
             }
             bool isSuccess = _loop.SaveLoopbackState();
             return isSuccess;
        }
    }
}
